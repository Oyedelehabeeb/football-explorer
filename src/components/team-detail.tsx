...
function SquadGroup({ position, players, season }: { position: string; players: SquadPlayer[]; season: number }) {
  return <section className="squad-group"><header><span>{position}</span><small>{players.length}</small></header><div>{players.map((player) => <Link to="/players/$playerId" params={{ playerId: String(player.id) }} search={{ season }} key={player.id}><img src={player.photo} alt="" loading="lazy" /><div><h3>{player.name}</h3><p>{player.age ? `${player.age} years` : position}</p></div><strong>{player.number ?? '—'}</strong></Link>)}</div></section>
}
