...
export function TeamExplorer({ result, query, leagueId }: { result: TeamsResult; query: string; leagueId: number }) {
  ...
  const submitSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (nextQuery.length > 0 && nextQuery.length < 3) return
    void navigate({ search: { q: nextQuery || undefined, league: leagueId }, replace: true, resetScroll: false })
  }
  return <main id="main-content" className="teams-page"><section className="teams-hero"><div className="page-shell"><span className="eyebrow"><span /> CLUB DIRECTORY</span><div><section><h1>Find the club.<br /><em>Know the story.</em></h1><p>Explore the teams shaping Europe’s leading leagues, from identity and home ground to squad and performance.</p></section><div className="teams-hero-mark"><Users aria-hidden="true" /><strong>7</strong><span>Priority leagues</span></div></div></div></section>
    <section className="team-directory page-shell" aria-labelledby="team-directory-title"><header><div><span className="section-number">01 / TEAM INDEX</span><h2 id="team-directory-title">{query ? `Results for “${query}”` : league.name}</h2></div><p>{result.ok ? `${teams.length} teams` : 'Data unavailable'}</p></header>
      <form className="team-search" role="search" onSubmit={submitSearch}><Search aria-hidden="true" /><label className="sr-only" htmlFor="team-search-input">Search teams</label><Input id="team-search-input" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Search any team — minimum 3 characters" autoComplete="off" /><button type="submit" disabled={input.trim().length > 0 && input.trim().length < 3}>Search</button></form>
      <div className="priority-league-tabs" role="group" aria-label="Choose a priority league">{PRIORITY_LEAGUES.map((item) => <button type="button" aria-pressed={!query && leagueId === item.id} key={item.id} className={!query && leagueId === item.id ? 'is-active' : ''} onClick={() => chooseLeague(item.id)}><span>{item.country}</span>{item.name}</button>)}</div>
