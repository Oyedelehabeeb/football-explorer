...
export function CompetitionExplorer({ result, search }: CompetitionExplorerProps) {
  ...
  return <main id="main-content" className="competitions-page">
    <section className="competitions-hero"><div className="page-shell"><span className="eyebrow"><span /> The global game</span><div className="competitions-hero-grid"><div><h1>Find your<br /><em>competition.</em></h1><p>Explore the leagues and cups shaping football across every region, season and level of the game.</p></div><div className="competition-index-stats"><div><strong>{competitions.length.toLocaleString()}</strong><span>Available competitions</span></div><div><strong>{countries.length}</strong><span>Countries</span></div><div><strong>{leagueCount}</strong><span>Leagues</span></div><div><strong>{cupCount}</strong><span>Cups</span></div></div></div></div></section>
    ...
    <section className="competition-directory page-shell" aria-labelledby="competition-directory-title"><div className="directory-heading"><div><span className="section-number">02 / COMPETITION INDEX</span><h2 id="competition-directory-title">The football directory</h2></div><p>{filtered.length.toLocaleString()} {filtered.length === 1 ? 'competition' : 'competitions'}</p></div>
      ...
      {!result.ok ? <div className="error-state" role="alert"><ShieldAlert aria-hidden="true" /><div><h2>Competition index unavailable</h2><p>{result.message}</p></div></div> : visible.length === 0 ? <div className="empty-state"><div className="empty-icon"><Search aria-hidden="true" /></div><h2>No competitions found</h2><p>Try a broader search or clear one of the filters.</p></div> : <><div className="competition-list">{visible.map((competition) => {
        ...
        return <article className="competition-row" key={competition.league.id}><div className="competition-rank">{String(competition.league.id).padStart(3, '0')}</div><div className="competition-logo"><img src={competition.league.logo} alt="" loading="lazy" /></div><div className="competition-name"><span>{competition.country.flag && <img src={competition.country.flag} alt="" loading="lazy" />}{competition.country.name}</span><h3>{competition.league.name}</h3></div><div className="competition-season"><CalendarRange aria-hidden="true" /><div><span>Latest season</span><strong>{season?.year ?? '—'}</strong></div></div><div className="competition-coverage"><Database aria-hidden="true" /><div><span>Coverage</span><strong>{coverageCount(season)} data areas</strong></div></div><span className={`competition-kind is-${competition.league.type.toLowerCase()}`}><Trophy aria-hidden="true" />{competition.league.type}</span></article>
